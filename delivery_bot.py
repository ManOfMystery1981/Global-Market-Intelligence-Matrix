# delivery_bot.py - Full version with article integration
import os
import sys
import base64
import resend
...
# In delivery_bot.py - Add this function to get article path
def get_article_path():
    """Get the article path from environment variable or find the latest."""
    # Check if path is set via environment
    env_path = os.environ.get("ANALYST_ARTICLE_PATH")
    if env_path and os.path.exists(env_path):
        return env_path
    
    # Otherwise, find the latest article
    import glob
    articles = glob.glob("analyst_article_*.md")
    if articles:
        return sorted(articles)[-1]
    return None

# Then in generate_enhanced_report_pdf, call this:
article_path = get_article_path()
if article_path:
    with open(article_path, 'r') as f:
        article_text = f.read()
    # Process and add to PDF...

# --- ENHANCED PDF GENERATION WITH ARTICLE ---
...
def send_report_email(customer_email, pdf_data=None, is_test=False):
    ...
    email_payload = {"from": from_email, "to": [customer_email], "subject": subject, "html": html_body}
    if pdf_data:
        pdf_base64 = base64.b64encode(pdf_data).decode('utf-8')
        email_payload["attachments"] = [{"filename": "Global_Software_Trends_Report.pdf", "content": pdf_base64, "content_type": "application/pdf"}]
...
def dispatch_secure_fulfillment_package(customer_email):
    ...
    try:
        ...
        pdf_data = generate_enhanced_report_pdf(
            customer_email, trend_data, metrics_data, codebase_stats,
            market_data, chart_images, os_data
        )
